...
*   **設定:** `IBufferManagerOptions` (実装は `BufferManagerOptions`) を介して行う。DIあり/なし両対応。
    ...
    public interface IProviderOptionsBuilderBase
    {
        ...
        IProviderOptionsBuilderBase SetLifecycleHooks<TElement, TBufferImplementation>(IBufferLifecycleHooks<TBufferImplementation, TElement> hooks)
            where TBufferImplementation : class, IBuffer<TElement>
